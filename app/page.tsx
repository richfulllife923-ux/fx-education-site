...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <section className="container-page pb-16 sm:pb-24" aria-labelledby="tutto-human-heading">
        <div className="card border-primary/60 bg-[linear-gradient(135deg,rgba(59,130,246,0.16),rgba(21,28,46,0.88)_44%,rgba(11,16,32,0.96))] p-6 sm:p-8 lg:p-10">
          <p className="eyebrow">TUTTO + HUMAN</p>
          <h2 id="tutto-human-heading" className="text-3xl font-bold leading-snug text-text-primary sm:text-4xl">
            なぜEAではなく、Indicatorなのか
          </h2>
          <p className="mt-8 border-l-2 border-primary pl-5 text-2xl font-bold leading-relaxed text-text-primary sm:text-3xl">
            市場を観測するのはTUTTO。<br />
            最後に判断するのは人間。
          </p>
          <div className="mt-8 grid gap-6 text-base leading-8 text-text-secondary lg:grid-cols-2 lg:gap-10">
          ...
          <ol aria-label="観測から人間の最終判断までの流れ" className="my-10 grid grid-cols-1 gap-8 border-y border-border py-8 lg:grid-cols-5 lg:gap-6">
            {["OBSERVE", "VERIFY", "THINK", "RISK MANAGEMENT", "DECISION"].map((step, index) => (
              <li key={step} className="relative min-w-0 rounded-card border border-primary/40 bg-background/55 p-4">
                <span className="font-display text-xs font-black text-primary">0{index + 1}</span>
                <p className="mt-3 font-display text-sm font-bold leading-6 text-text-primary">{step}</p>
                {index < 4 && (
                  <ArrowRight size={18} strokeWidth={1.8} aria-hidden="true" className="absolute -bottom-6 left-1/2 -translate-x-1/2 rotate-90 text-primary lg:-right-5 lg:bottom-auto lg:left-auto lg:top-1/2 lg:-translate-y-1/2 lg:translate-x-0 lg:rotate-0" />
                )}
              </li>
            ))}
          </ol>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              <h3 className="text-2xl font-bold text-text-primary sm:text-3xl">人間プラスの法則</h3>
              <div className="mt-5 space-y-4 text-base leading-8 text-text-secondary">
                ...
                <p>
                  これをTUTTOでは「人間プラスの法則」と考えています。
                </p>
                <p>
                  TUTTOは取引を自動化するためのものではありません。
                </p>
                <p>
            ...
            <div className="rounded-card border border-primary/40 bg-background/55 p-6 sm:p-8">
              <p className="text-lg font-semibold leading-8 text-text-primary">
                機械に任せきらない。<br />
                人間だけにも頼らない。
              </p>
              <p className="mt-6 font-display text-3xl font-black text-text-primary sm:text-4xl">
                TUTTO <span className="text-primary">+</span> Human
              </p>
              <p className="mt-3 text-lg font-bold text-text-primary">人間プラスの法則。</p>
            </div>
          </div>
          <p className="mt-8 border-t border-border pt-5 text-xs leading-6 text-text-secondary">
            TUTTOは市場構造の観測・研究・教育・分析補助を目的としたIndicatorです。
            特定の取引を推奨するものではなく、
            投資助言、売買指示、利益・勝率・将来価格を保証するものではありません。
          </p>
        </div>
      </section>
